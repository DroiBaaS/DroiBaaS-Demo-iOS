...
- (void)demoBackgroundCreateObject {
    
    Student *student = [[Student alloc] init];
    student.name = @"Jon_Back";
    student.age = 250;
...
- (void)demoUpdateObjectInBackground {
    
    DroiQuery *query = [[DroiQuery create] queryByClass:[Student class]];
    DroiCondition *cond = [DroiCondition eq:@"name" andArg2:@"Jon"];
    query = [query whereStatement:cond];
    [query runQueryInBackground:^(NSArray *result, DroiError *error) {
        if ([result count] > 0) {
            Student *stu = [result objectAtIndex:0];
        stu.name = @"JonUUUUUUUUUUUUUUU";
            [stu saveInBackground:^(BOOL result, DroiError *error) {
                if (result) {
                    DroiLog(@"更新student成功! name:%@",stu.name);
                }
                else{
                    DroiLog(@"更新失败%@",error.message);
                }
...
- (void)demoCreateObjectAndFile {
    ...
    DroiFile *avatar = [DroiFile fileWithFilePath:filePath];
...
- (void)demoBatchFetch {
    ...
    [self createStudentsForDemo:^(id object) {
        ...
        NSArray *fetchStudentIds1 = @[@"52d34d54b1d0ae9a44c35bfe",@"64bc45c393e6044bf001016d",@"21f44bed8b3a1ed9c5d32873"];
        DroiQuery *query = [[DroiQuery create] queryByName:@"Student"];
        query = [query whereStatement:[DroiCondition selectIn:@"_Id" withItems:fetchStudentIds1]];
        [query runQueryInBackground:^(NSArray *result, DroiError *err) {
            if (err.isOk) {
